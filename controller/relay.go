	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		if channelErr != nil {
			...
			processChannelError(c, *types.NewChannelError(channel.Id, channel.Type, channel.Name,
				channel.ChannelInfo.IsMultiKey, common.GetContextKeyString(c, constant.ContextKeyChannelKey),
				channel.GetAutoBan()), channelErr, willRetry)
			if willRetry {
				continue
			}
			break
		...
		if newAPIError == nil {
			newAPIError = helper.StreamStatusError(relayInfo)
		}
		...
		willRetry := shouldRetryRelayAttempt(c, relayInfo, newAPIError, common.RetryTimes-retryParam.GetRetry())
...
func shouldRetry(c *gin.Context, openaiErr *types.NewAPIError, retryTimes int) bool {
	...
	attemptOutcome := service.ClassifyRelayAttempt(openaiErr)
	if types.IsSkipRetryError(openaiErr) && attemptOutcome != service.RelayAttemptChannelUnavailable {
		return false
	}
	if types.IsChannelError(openaiErr) {
		return true
	}
	if attemptOutcome == service.RelayAttemptModelUnavailable {
		return true
	}
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, retried ...bool) {
	logger.LogError(c, fmt.Sprintf("channel error (channel #%d, status code: %d): %s", channelError.ChannelId, err.StatusCode, common.LocalLogPreview(err.Error())))
	// 不要使用context获取渠道信息，异步处理时可能会出现渠道信息不一致的情况
	// do not use context to get channel info, there may be inconsistent channel info when processing asynchronously
	if !operation_setting.SelfUseModeEnabled && service.ShouldDisableChannel(err) && channelError.AutoBan {
		gopool.Go(func() {
	...
	if constant.ErrorLogEnabled && types.IsRecordErrorLog(err) {
		...
		service.AppendRelayFailureTrace(c, adminInfo, len(retried) > 0 && retried[0])
